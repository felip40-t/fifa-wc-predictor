...
from fifa_predictor.model import simulate
from fifa_predictor.model.simulate import _implied_rates_from_odds_row, _row_value
...
def test_simulate_match_degenerate_matrix_returns_that_scoreline() -> None:
    ...
    for _ in range(20):
        assert simulate.simulate_match(matrix, rng) == (2, 1)
...
def test_simulate_match_empirical_distribution_matches_matrix() -> None:
    ...
    n = 50_000
    for _ in range(n):
        h, a = simulate.simulate_match(matrix, rng)
        counts[h, a] += 1
...
def test_simulate_match_returns_python_ints() -> None:
    ...
    matrix = np.zeros((3, 3))
    matrix[1, 2] = 1.0
    h, a = simulate.simulate_match(matrix, np.random.default_rng(1))
...
def test_goals_round_rounds_up_only_at_seven_tenths() -> None:
    ...
    assert simulate.goals_round(0.5) == 0
    assert simulate.goals_round(0.69) == 0
    assert simulate.goals_round(0.7) == 1
    assert simulate.goals_round(0.71) == 1
    assert simulate.goals_round(0.45) == 0
    assert simulate.goals_round(2.7) == 3
    assert simulate.goals_round(3.12) == 3
...
def test_result_consistent_mode_favourite_never_returns_a_draw() -> None:
    ...
    score = simulate.result_consistent_mode(matrix)
...
def test_game_outcomes_keys_and_analytical_probabilities() -> None:
    ...
    result = simulate._game_outcomes(lh=lh, la=la, rho=rho, max_goals=10)
...
def test_implied_rates_from_odds_row_returns_positive_rates() -> None:
    ...
    lh, la, rho, residual_norm = simulate._implied_rates_from_odds_row(
...
def test_simulate_games_from_odds_is_deterministic(tmp_path) -> None:
    ...
    first = simulate.simulate_games_from_odds(
        str(odds_csv), output_csv_path=str(tmp_path / "a.csv")
    )
    second = simulate.simulate_games_from_odds(
...
def test_headline_modest_lead_below_margin_uses_exact() -> None:
    ...
    out = simulate.select_headline_score(0.42, 0.32, 0.26, "2-1", "1-1")
    assert out == "1-1"
...
def test_headline_boundary_at_margin_commits_to_result() -> None:
    ...
    out = simulate.select_headline_score(0.45, 0.30, 0.25, "2-1", "1-1")
    assert out == "2-1"


def test_headline_away_favorite_symmetric() -> None:
    # Away clearly favored: same behavior as a home favorite.
    out = simulate.select_headline_score(0.15, 0.20, 0.65, "0-2", "0-1")
    assert out == "0-2"
